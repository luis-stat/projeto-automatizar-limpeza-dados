...
class DataCleaner:
    ...
    def clean_column(self, series: pd.Series, column_type: str, column_name: str) -> Tuple[pd.Series, Dict[str, Any]]:
        cleaning_stats = {
        ...
        if column_type in ['TEXTO_LIVRE', 'CATEGORICO_NOMINAL']:
            ...
            cleaned_series, fuzzy_stats = self.fuzzy_correction(cleaned_series)
    ...
    def clean_dataset(self, df: pd.DataFrame, type_predictions: Dict[str, str]) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        cleaned_df = df.copy()
        overall_report = {
            'columns_cleaned': {},
